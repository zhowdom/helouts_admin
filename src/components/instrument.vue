    ...
    <div class="indexBoxInner rela">
      ...
      <div class="mainWrapper clearfix">
          ...
          <div v-show="vshowFlag" class="leftMain fl" :class="{ 'animate__animated animate__slideInLeft':openStatus, 'animate__animated animate__fadeInLeft':closeStatus }">
            ...
            <!--5-->
            <div class="energyStatistic bdbox commonTitlebox comBoderAndBg rela" style="width:520px;height:235px;border: none;margin-top:0!important;">
                <div class="main1 flex-row" style="width:504px;">
                  <div class="mod1 flex-col"></div>
                  <span class="txt1">运行状态</span>
                  <span class="word1">Running&nbsp;state</span>
                  <div class="mod2 flex-col" style="margin-left:184px;"></div>
                  <div class="mod3 flex-col"></div>
                </div>
                <div class="linerStyle" style="margin-top:5px;margin-left:16px;width:484px;"></div>
                
                <div id="echrMain3Pie3" style="width: 420px;height: 170px;"></div>
            </div>

            <!--4-->
            <div class="energyStatistic bdbox commonTitlebox comBoderAndBg" style="width:520px;height:235px;">
                <div class="main1 flex-row" style="width:504px;">
                  <div class="mod1 flex-col"></div>
                  <span class="txt1">仪器设备资产统计</span>
                  <span class="word1" style="margin-left:63px;">（单位：万元）</span>
                  <div class="mod2 flex-col" style="margin-left:130px;"></div>
                  <div class="mod3 flex-col"></div>
                </div>
                <div class="linerStyle" style="margin-top:5px;margin-left:16px;width:484px;"></div>
                <div id="echrMain" style="width: 490px;height: 180px;"></div>
            </div>
            
            <!--5-->
            <div class="energyStatistic bdbox commonTitlebox comBoderAndBg" style="width:520px;height:235px;">
                <div class="main1 flex-row" style="width:525px;">
                  <div class="mod1 flex-col"></div>
                  <span class="txt1">科室仪器占比</span>
                  <span class="word1" style="margin-left:40px">Proportion&nbsp;of&nbsp;instruments</span>
                ...
                <div class="nandBox-l fl">
                    <div class="nandChartbox" style="width:200px;height:180px;">
                      <div id="nandMain" style="width:500px;height:180px;"></div>
                    </div>
                  </div>
            </div>
            ...
            <div class="energyStatistic bdbox commonTitlebox comBoderAndBg" style="width:520px;height:233px;">
                <div class="main1 flex-row" style="width:545px;">
                  <div class="mod1 flex-col"></div>
                  <span class="txt1">仪器列表</span>
                  <span class="word1">Instrument&nbsp;list</span>
                  <div class="mod2 flex-col" style="margin-left:182px;"></div>
                  <div class="mod3 flex-col"></div>
                </div>
                <div class="linerStyle" style="margin-top:5px;margin-left:16px;width:481px;"></div>
                <div class="darebox" style="overflow-y: scroll;">
                  <li v-for="(item, index) in indexData.statusList" :key="index" :class="{'red': item.status == 0, 'green': item.status == 1, 'orange': item.status == 2}">
                    <span style="display:inline-block;width:90%;" class="eplise">{{item.name}}</span>
                  </li>
                </div>
            </div>
          ...
          <div v-show="vshowFlag" class="rightMain fr" :class="{ 'animate__animated animate__slideInRight':openStatus, 'animate__animated animate__fadeInRight':closeStatus }">
            ...
            <!--来访统计-->
            <div class="visitStatistic bdbox commonTitlebox comBoderAndBg fr nblast" style="height:968px;border:none;border-top-left-radius:0;margin-top:0!important;">
              <div class="main1 flex-row">
                <div class="mod1 flex-col"></div>
                <span class="txt1">仪器设备使用率</span>
                <span class="word1" style="margin-left:60px;">Utilization&nbsp;rate</span>
                <div class="mod2 flex-col" style="margin-left:132px;"></div>
                <div class="mod3 flex-col"></div>
              </div>

              <div class="linerStyle" style="margin-top:5px;margin-left:16px;"></div>

              <div class="oneBoxr fl n2n clearfix" style="width:500px;margin-top:10px;">
                  <div class="nandBox-r sdcv" style="width:100%;">
                    <div class="nandBox-rT clearfix" style="background:#141d34;border-bottom: 1px solid #121a32;"><div class="op eplise" style="width:70px">排名</div><div class="no eplise">科室</div><div class="name eplise">设备名</div><div class="time eplise">使用时长</div></div>
                    <div v-if="indexData.useRankList" class="nandBox-rB">
                      <ul>
                        <li v-for="(item, index) in indexData.useRankList" :key="index" class="clearfix" @click="showdidialog(item.deviceCode)">
                          <div class="op eplise" style="width:70px">{{index+1}}</div>
                          <el-tooltip class="item" effect="dark" :content="item.spaceName" placement="top-start">
                            <div class="no eplise">{{item.spaceName||'-'}}</div>
                          </el-tooltip>
                          <el-tooltip class="item" effect="dark" :content="item.deviceName" placement="top-start">
                            <div class="name eplise">{{item.deviceName}}</div>
                          </el-tooltip>
                          <div class="time eplise">{{item.hours}}h</div>
                        </li>
              ...
              <div class="clearfix" style="margin-bottom:10px;"></div>
                  ...
                  <div class="llastbox fl" style="margin-bottom:10px;">
                    <h3>校准设备</h3>
                    <ul v-if="indexData.calibrationList" style="height:264px;overflow-y: scroll;overflow-x: hidden;">
                      <li v-for="(item, index) in indexData.calibrationList" :key="index" @click="showdidialog(item.deviceCode)">
                        <span class="lstatus" :class="{'wills': item.status == 0}" style="margin-bottom:9px;">{{item.status == 1 ? '已校准' : '未校准'}}</span>
                        <el-tooltip class="item" effect="dark" :content="item.deviceName" placement="top-start">
                          <span class="lname eplise">{{item.deviceName}}</span>
                        </el-tooltip>
                          <span class="ldesc">
                            <i>倒计时：{{item.leftDays}}天</i>
                            <el-tooltip class="item" effect="dark" :content="item.deviceCode" placement="top-start">
                              <i>｜编号：{{item.deviceCode}}</i>
                            </el-tooltip>
                          </span>
                      </li>
                    </ul>
                  ...
                  <div class="llastbox fl">
                    <h3>期间核准设备</h3>
                    <ul style="height:308px;overflow-y: scroll;overflow-x: hidden;">
                      <li v-for="(item, index) in indexData.periodCalibrationList" :key="index" @click="showdidialog(item.deviceCode)">
                        <span class="lstatus" :class="{'wills': item.status == 0}" style="margin-bottom:9px;">{{item.status == 1 ? '已校准' : '未校准'}}</span>
                        <el-tooltip class="item" effect="dark" :content="item.deviceName" placement="top-start">
                          <span class="lname">{{item.deviceName}}</span>
                        </el-tooltip>
                        <span class="ldesc">
                          <i>倒计时：{{item.leftDays}}天</i>
                          <el-tooltip class="item" effect="dark" :content="item.deviceCode" placement="top-start">
               ...
               <!--左侧悬浮菜单-->
              <div class="cmABSMenu cmABSMenuv2 abs bdbox clearfix needHIDE" style="height:52px;left:42px;top:10px;">
      ...
      <el-dialog
        ...
        custom-class="dzDialogStyle_eqp rela">
        <div class="dzDialogHeader clearfix abs">
          <div class="fl"></div>
          <div class="rightBBar">
            109实验室-基本情况
        ...
        <div class="clearfix wnlistbox">
          <div class="clearfix eechliv2 rela fl">
            <div><i class="cm_daoTriangle_bg abs"></i><span class="eplise" style="color:#fff;padding-left:10px;padding-top:3px;display:inline-block;">温度</span></div>
            <div style="color:#32C5FF;padding-left:10px;">27­°C</div>
          </div>
          <div class="clearfix eechliv2 rela fr">
            <div><i class="cm_daoTriangle_bg abs"></i><span class="eplise" style="color:#fff;padding-left:10px;padding-top:3px;display:inline-block;">压力</span></div>
            <div style="color:#32C5FF;padding-left:10px;">0.7PA</div>
          </div>
          <div class="clearfix eechliv2 rela fl">
            <div><i class="cm_daoTriangle_bg abs"></i><span class="eplise" style="color:#fff;padding-left:10px;padding-top:3px;display:inline-block;">湿度</span></div>
            <div style="color:#32C5FF;padding-left:10px;">80%</div>
          </div>
          <div class="clearfix eechliv2 rela fr">
            <div><i class="cm_daoTriangle_bg abs"></i><span class="eplise" style="color:#fff;padding-left:10px;padding-top:3px;display:inline-block;">PM2.5</span></div>
            <div style="color:#32C5FF;padding-left:10px;">48</div>
          </div>
          <div class="clearfix eechliv2 rela fl">
            <div><i class="cm_daoTriangle_bg abs"></i><span class="eplise" style="color:#fff;padding-left:10px;padding-top:3px;display:inline-block;">值班人</span></div>
            <div style="color:#32C5FF;padding-left:10px;">张三 I  15915805678</div>
          </div>
          <div class="clearfix eechliv2 rela fr">
            <div><i class="cm_daoTriangle_bg abs"></i><span class="eplise" style="color:#fff;padding-left:10px;padding-top:3px;display:inline-block;">安全负责人</span></div>
            <div style="color:#32C5FF;padding-left:10px;">张三 I  15915805678</div>
          </div>
        </div>
        ...
        <div class="hwcdBox" style="padding:10px;background:#2a3044;border-radius:4px;margin-top:10px;">
          <div class="clearfix" style="border-bottom:1px solid #1f253d;padding-bottom:5px;">
            <h3 class="fl" style="color:#32C5FF;">恒温恒湿空调机组</h3>
            <span class="fr" style="color:#fff;font-size:12px;">运行状态：<i style="color:#32C5FF;font-style:normal;">运行</i></span>
          </div>
          <div style="color:#fff;padding:10px 0;font-size:13px;line-height:24px;">机组设计灵活，送风方式多样化，冷量范围广，种类型号多，多种环保冷媒可选择，满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境满足各种环境低漏风率</div>
          ...
          <div class="mabboxB-l mabboxB-l_v2 clearfix" style="margin-top:5px!important;">
            <div class="fl eacc rela" style="margin-left:14px;height:57px!important;background:#1e2437;">
              <div class="topPP abs clearfix" style="width:100%;border-bottom:1px solid #131931;">
                <i class="fl"></i><span class="fl">电源</span>
              </div>
              <p style="line-height:80px;">15A</p>
            </div>
            <div class="fl eacc rela" style="margin-left:25px;margin-right:25px;height:57px!important;background:#1e2437;">
              <div class="topPP abs clearfix" style="width:100%;border-bottom:1px solid #131931;">
                <i class="fl"></i><span class="fl">电压</span>
              </div>
              <p style="line-height:80px;">220V</p>
            </div>
            <div class="fl eacc rela" style="height:57px!important;background:#1e2437;">
              <div class="topPP abs clearfix" style="width:100%;border-bottom:1px solid #131931;">
                <i class="fl"></i><span class="fl">运行时长</span>
              </div>
              <p style="line-height:80px;">22h</p>
            </div>
          </div>
        </div>
  ...
  methods: {
    queryIndexData () {
      return new Promise((resolve, reject) => {
        this.$axios.get('/api/digital/GetInstrumentData').then(({data}) => {
          if (data.code == 200) {
    ...
    handleCloseEirn () {
      ...
      blu_event("仪器设备弹窗关闭", 'null')
    },
    myEchartsNanding () {
      // 通过id获取获取画布
      var dom = document.getElementById("nandMain");
      ...
      option = {
          ...
          legend: [{
              ...
              data: [...this.indexData.classCount && this.indexData.classCount.map(v => v.className)].slice(0,3)
          },
          {
              left: 'center',
              icon: 'circle',
              ...
              data: [...this.indexData.classCount && this.indexData.classCount.map(v => v.className)].slice(3,this.indexData.classCount.length)
          }
          ],
          series: [
              {
                  ...
                  emphasis: {
                      label: {
                          show: false, // 这里是饼图连着线的指示文字，
                          color: 'red' // 可设置颜色
                      }
                  },
                  data: this.indexData.classCount.map((v, i) => {
                    let colorArr = ['#4B7DF8FF', '#FD5B65FF', '#FD9544FF', '#FFE8A9FF', '#21BF6BFF', '#4BD5F7FF']
                    return {
                      value: v.count, name: v.className, itemStyle: { color: colorArr[i] }
                    }
                  })
              }
          ]
    ...
    myEcharts3Pie3 () {
      var myChart = echarts.init(document.getElementById('echrMain3Pie3'))
      ...
      var option = {
          ...
          series: [{
              ...
              data: this.indexData.runningStatus && this.indexData.runningStatus.map((v, index) => {
                let colorArr = ['#fbe9b0', '#3659da', '#567bee', '#74d2f3']
                return {
                  value: v.counts,
                  name: v.statusName,
...
.darebox li.orange::after{
  background: #FFF776FF;
}
.darebox li.green::after{
  background: #29FF3FFF;
}
</style>
<style scoped>
...
.llastbox li .ldesc i.hide{
  color: #202a44;
}
.nblast .n2n .sdcv{
  height:auto;
}
.nblast .sdcv .nandBox-rT{
  padding-top: 14px;
  padding-bottom: 14px;
}
.nblast .n2n .sdcv ul{
